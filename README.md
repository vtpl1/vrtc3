<h1 align="center">
  ...
  ![vrtc3](assets/logo.gif)
  <br>
  [![stars](https://img.shields.io/github/stars/vtpl1/vrtc3?style=flat-square&logo=github)](https://github.com/vtpl1/vrtc3/stargazers) 
  [![docker pulls](https://img.shields.io/docker/pulls/alexxit/vrtc3?style=flat-square&logo=docker&logoColor=white&label=pulls)](https://hub.docker.com/r/alexxit/vrtc3) 
  [![releases](https://img.shields.io/github/downloads/vtpl1/vrtc3/total?color=blue&style=flat-square&logo=github)](https://github.com/vtpl1/vrtc3/releases)
  [![goreport](https://goreportcard.com/badge/github.com/vtpl1/vrtc3)](https://goreportcard.com/report/github.com/vtpl1/vrtc3)
</h1>
...
- zero-dependency and zero-config [small app](#vrtc3-binary) for all OS (Windows, macOS, Linux, ARM)
...
- support H265 for WebRTC in browser (Safari only, [read more](https://github.com/vtpl1/Blog/issues/5))
...
- can be [integrated to](#module-api) any smart home platform or be used as [standalone app](#vrtc3-binary)
...
* [Fast start](#fast-start)
  * [vrtc3: Binary](#vrtc3-binary)
  * [vrtc3: Docker](#vrtc3-docker)
  * [vrtc3: Home Assistant Add-on](#vrtc3-home-assistant-add-on)
  * [vrtc3: Home Assistant Integration](#vrtc3-home-assistant-integration)
  * [vrtc3: Dev version](#vrtc3-dev-version)
...
* [Projects using vrtc3](#projects-using-vrtc3)
...
1. Download [binary](#vrtc3-binary) or use [Docker](#vrtc3-docker) or Home Assistant [Add-on](#vrtc3-home-assistant-add-on) or [Integration](#vrtc3-home-assistant-integration)
...
The Docker container [`alexxit/vrtc3`](https://hub.docker.com/r/alexxit/vrtc3) supports multiple architectures including `amd64`, `386`, `arm64`, and `arm`. This container offers the same functionality as the [Home Assistant Add-on](#vrtc3-home-assistant-add-on) but is designed to operate independently of Home Assistant. It comes preinstalled with [FFmpeg](#source-ffmpeg), [ngrok](#module-ngrok), and [Python](#source-echo).

### vrtc3: Home Assistant Add-on

[![](https://my.home-assistant.io/badges/supervisor_addon.svg)](https://my.home-assistant.io/redirect/supervisor_addon/?addon=a889bffc_go2rtc&repository_url=https%3A%2F%2Fgithub.com%2Fvtpl1%2Fhassio-addons)

1. Install Add-On:
    - Settings > Add-ons > Plus > Repositories > Add `https://github.com/vtpl1/hassio-addons`
    - vrtc3 > Install > Start
2. Setup [Integration](#module-hass)

### vrtc3: Home Assistant Integration

[WebRTC Camera](https://github.com/vtpl1/WebRTC) custom component can be used on any [Home Assistant installation](https://www.home-assistant.io/installation/), including [HassWP](https://github.com/vtpl1/HassWP) on Windows. It can automatically download and use the latest version of vrtc3. Or it can connect to an existing version of vrtc3. Addon installation in this case is optional.

### vrtc3: Dev version

Latest, but maybe unstable version:

- Binary: [latest nightly release](https://nightly.link/vtpl1/vrtc3/workflows/build/master)
- Docker: `alexxit/vrtc3:master` or `alexxit/vrtc3:master-hardware` versions
- Hass Add-on: `vrtc3 master` or `vrtc3 master hardware` versions

## Configuration

- by default vrtc3 will search `vrtc3.yaml` in the current work directory
- `api` server will start on default **1984 port** (TCP)
- `rtsp` server will start on default **8554 port** (TCP)
- `webrtc` will use port **8555** (TCP/UDP) for connections
- `ffmpeg` will use default transcoding options

Configuration options and a complete list of settings can be found in [the wiki](https://github.com/vtpl1/vrtc3/wiki/Configuration).
...
**vrtc3** support different stream source types. You can config one or multiple links of any type as stream source.
...
- [webtorrent](#source-webtorrent) - WebTorrent source from another vrtc3
...
vrtc3 also support [play audio](#stream-to-camera) files and live streams on this cameras.
...
- **Dahua Doorbell** users may want to change backchannel [audio codec](https://github.com/vtpl1/vrtc3/issues/52)
...
- **Ubiquiti UniFi** users may want to disable HTTPS verification. Use `rtspx://` prefix instead of `rtsps://`. And don't use `?enableSrtp` [suffix](https://github.com/vtpl1/vrtc3/issues/81)
- **TP-Link Tapo** users may skip login and password, because vrtc3 support login [without them](https://drmnsamoliu.github.io/video.html)
...
- **HTTP-JPEG** (`image/jpeg`) - camera snapshot link, can be converted by vrtc3 to MJPEG stream
...
*[New in v1.5.0](https://github.com/vtpl1/vrtc3/releases/tag/v1.5.0)*
...
You can get any stream or file or device via FFmpeg and push it to vrtc3. The app will automatically start FFmpeg with the proper arguments when someone starts watching the stream.
...
All trascoding formats has [built-in templates](https://github.com/vtpl1/vrtc3/blob/master/internal/ffmpeg/ffmpeg.go): `h264`, `h265`, `opus`, `pcmu`, `pcmu/16000`, `pcmu/48000`, `pcma`, `pcma/16000`, `pcma/48000`, `aac`, `aac/16000`.
...
- You can use vrtc3 stream name as ffmpeg input (ex. `ffmpeg:camera1#video=h264`)
...
Read more about [hardware acceleration](https://github.com/vtpl1/vrtc3/wiki/Hardware-acceleration).
...
Exec source can run any external application and expect data from it. Two transports are supported - **pipe** (*from [v1.5.0](https://github.com/vtpl1/vrtc3/releases/tag/v1.5.0)*) and **RTSP**.
...
- [FFmpeg](https://ffmpeg.org/) - vrtc3 ffmpeg source just a shortcut to exec source
...
Check examples in [wiki](https://github.com/vtpl1/vrtc3/wiki/Source-Echo-examples).
...
*[New in v1.8.2](https://github.com/vtpl1/vrtc3/releases/tag/v1.8.2)*

Like `echo` source, but uses the built-in [expr](https://github.com/antonmedv/expr) expression language ([read more](https://github.com/vtpl1/vrtc3/blob/master/internal/expr/README.md)).
...
- HomeKit device can be paired with only one ecosystem. So, if you have paired it to an iPhone (Apple Home) - you can't pair it with Home Assistant or vrtc3. Or if you have paired it to vrtc3 - you can't pair it with iPhone
- HomeKit device should be in same network with working [mDNS](https://en.wikipedia.org/wiki/Multicast_DNS) between device and vrtc3

vrtc3 support import paired HomeKit devices from [Home Assistant](#source-hass). So you can use HomeKit camera with Hass and vrtc3 simultaneously. If you using Hass, I recommend pairing devices with it, it will give you more options.

You can pair device with vrtc3 on the HomeKit page. If you can't see your devices - reload the page. Also try reboot your HomeKit device (power off). If you still can't see it - you have a problems with mDNS.
...
*[New in v1.6.1](https://github.com/vtpl1/vrtc3/releases/tag/v1.6.1)*
...
*[New in v1.2.0](https://github.com/vtpl1/vrtc3/releases/tag/v1.2.0)*
...
*[New in v1.7.0](https://github.com/vtpl1/vrtc3/releases/tag/v1.7.0)*
...
*[New in v1.8.3](https://github.com/vtpl1/vrtc3/releases/tag/v1.8.3)*

Support streaming from [GoPro](https://gopro.com/) cameras, connected via USB or Wi-Fi to Linux, Mac, Windows. [Read more](https://github.com/vtpl1/vrtc3/tree/master/internal/gopro).
...
**WebRTC Cameras** (*from [v1.6.0](https://github.com/vtpl1/vrtc3/releases/tag/v1.6.0)*)
...
By default, the Home Assistant API does not allow you to get dynamic RTSP link to a camera stream. So more cameras, like [Tuya](https://www.home-assistant.io/integrations/tuya/), and possibly others can also be imported by using [this method](https://github.com/felipecrs/hass-expose-camera-stream-source#importing-home-assistant-cameras-to-vrtc3-andor-frigate).

#### Source: ISAPI

*[New in v1.3.0](https://github.com/vtpl1/vrtc3/releases/tag/v1.3.0)*
...
*[New in v1.6.0](https://github.com/vtpl1/vrtc3/releases/tag/v1.6.0)*
...
Source support load Roborock credentials from Home Assistant [custom integration](https://github.com/humbertogontijo/homeassistant-roborock). Otherwise, you need to log in to your Roborock account (MiHome account is not supported). Go to: vrtc3 WebUI > Add webpage. Copy `roborock://...` source for your vacuum and paste it to `vrtc3.yaml` config.
...
[WebRTC/WHEP](https://datatracker.ietf.org/doc/draft-murillo-whep/) - is replaced by [WebRTC/WISH](https://datatracker.ietf.org/doc/charter-ietf-wish/02/) standard for WebRTC video/audio viewers. But it may already be supported in some third-party software. It is supported in vrtc3.

**vrtc3**

This format is only supported in vrtc3. Unlike WHEP it supports asynchronous WebRTC connection and two way audio.

**openipc** (*from [v1.7.0](https://github.com/vtpl1/vrtc3/releases/tag/v1.7.0)*)

Support connection to [OpenIPC](https://openipc.org/) cameras.

**wyze** (*from [v1.6.1](https://github.com/vtpl1/vrtc3/releases/tag/v1.6.1)*)
...
**kinesis** (*from [v1.6.1](https://github.com/vtpl1/vrtc3/releases/tag/v1.6.1)*)
...
streams:
  ...
  webrtc-vrtc3:  webrtc:ws://192.168.1.123:1984/api/ws?src=camera1
...
This source can get a stream from another vrtc3 via [WebTorrent](#module-webtorrent) protocol.
...
By default, vrtc3 establishes a connection to the source when any client requests it. Go2rtc drops the connection to the source when it has no clients left.
...
1. Create empty stream in the `vrtc3.yaml`
2. Go to vrtc3 WebUI
3. Open `links` page for you stream
4. Select `camera+microphone` or `display+speaker` option
5. Open `webrtc` local page (your vrtc3 **should work over HTTPS!**) or `share link` via [WebTorrent](#module-webtorrent) technology (work over HTTPS by default)
...
vrtc3 support play audio files (ex. music or [TTS](https://www.home-assistant.io/integrations/#text-to-speech)) and live streams (ex. radio) on cameras with [two way audio](#two-way-audio) support (RTSP/ONVIF cameras, TP-Link Tapo, Hikvision ISAPI, Roborock vacuums, any Browser).
...
- you can check camera codecs on the vrtc3 WebUI info page when the stream is active
...
- you will see one active producer and one active consumer in vrtc3 WebUI info page during streaming

### Publish stream

*[New in v1.8.0](https://github.com/vtpl1/vrtc3/releases/tag/v1.8.0)*
...
**Important!** vrtc3 passes requests from localhost and from unix socket without HTTP authorisation, even if you have it configured! It is your responsibility to set up secure external access to API. If not properly configured, an attacker can gain access to your cameras and even your server.

[API description](https://github.com/vtpl1/vrtc3/tree/master/api).
...
- you can change API `base_path` and host vrtc3 on your main app webserver suburl
...
api:
  ...
  tls_key: |         # default "", PEM-encoded private key for HTTPS
    ...
    ...
    -----END PRIVATE KEY-----
  unix_listen: "/tmp/vrtc3.sock"  # default "", unix socket listener for API
```
...
By default vrtc3 provide RTSP-stream with only one first video and only one first audio. You can change it with the `default_query` setting:
...
In most cases [WebRTC](https://en.wikipedia.org/wiki/WebRTC) uses direct peer-to-peer connection from your browser to vrtc3 and sends media data via UDP.
...
But about 10-20% of users may need to configure additional settings for external access if **mobile phone** or **vrtc3 server** behing [Symmetric NAT](https://tomchen.github.io/symmetric-nat-test/).
...
- add `stun` word and external port to YAML config
  - vrtc3 automatically detects your external address with STUN-server
...
If you have personal [VPS](https://en.wikipedia.org/wiki/Virtual_private_server), you can install TURN server (e.g. [coturn](https://github.com/coturn/coturn), config [example](https://github.com/vtpl1/WebRTC/wiki/Coturn-Example)).
...
- Video stream from HomeKit camera to RTSP/WebRTC/MP4/etc. will be transmitted via vrtc3
...
- Get any remote [vrtc3 source](#source-webtorrent) via [WebTorrent](https://webtorrent.io/) technology

Securely and free. You do not need to open a public access to the vrtc3 server. But in some cases (Symmetric NAT) you may need to set up external access to [WebRTC module](#module-webrtc).

To generate sharing link or incoming link - goto vrtc3 WebUI (stream links page). This link is **temporary** and will stop working after vrtc3 is restarted!

You can create permanent external links in vrtc3 config:

```yaml
webtorrent:
  shares:
    super-secret-share:  # share name, should be unique among all vrtc3 users!
      pwd: super-secret-password
      src: rtsp-dahua1   # stream name from streams section
```

Link example: https://alexxit.github.io/vrtc3/#share=02SNtgjKXY&pwd=wznEQqznxW&media=video+audio
...
- you may need external access for two different things:
  ...
  - vrtc3 web interface, so you need tunnel API HTTP port (ex. 1984)
...
vrtc3 will automatically get your external TCP address (if you enable it in ngrok config) and use it with WebRTC connection (if you enable it in webrtc config).
...
You need to create `ngrok.yaml` config file and add it to vrtc3 config:
...
tunnels:
  api:
    addr: 1984  # use the same port as in vrtc3 config
    proto: http
    basic_auth:
      - admin:password  # you can set login/pass for your web interface
  webrtc:
    addr: 8555  # use the same port as in vrtc3 config
    proto: tcp
```
...
The best and easiest way to use vrtc3 inside the Home Assistant is to install the custom integration [WebRTC Camera](#vrtc3-home-assistant-integration) and custom lovelace card.

But vrtc3 is also compatible and can be used with [RTSPtoWebRTC](https://www.home-assistant.io/integrations/rtsp_to_webrtc/) built-in integration.
...
2. Camera [any source](#module-streams) => [vrtc3 config](#configuration) => [Generic Camera](https://www.home-assistant.io/integrations/generic/)
   - Install any [vrtc3](#fast-start)
   - Add your stream to [vrtc3 config](#configuration)
...
3. `Camera Entity` or `Camera URL` => [WebRTC Camera](https://github.com/vtpl1/WebRTC) => Technology: `WebRTC/MSE/MP4/MJPEG`, codecs: `H264/H265/AAC/PCMU/PCMA/OPUS`, best latency, best compatibility.
   - Install and add [WebRTC Camera](https://github.com/vtpl1/WebRTC) custom integration
   - Use WebRTC Camera custom lovelace card

You can add camera `entity_id` to [vrtc3 config](#configuration) if you need transcoding:
...
**PS.** Default Home Assistant lovelace cards don't support 2-way audio. You can use 2-way audio from [Add-on Web UI](https://my.home-assistant.io/redirect/supervisor_addon/?addon=a889bffc_go2rtc&repository_url=https%3A%2F%2Fgithub.com%2Fvtpl1%2Fhassio-addons). But you need use HTTPS to access the microphone. This is a browser restriction and cannot be avoided.

**PS.** There is also another nice card with vrtc3 support - [Frigate Lovelace Card](https://github.com/dermotduffy/frigate-hass-card).
...
2. Camera snapshots in MP4 format (single frame), can be sent to [Telegram](https://github.com/vtpl1/vrtc3/wiki/Snapshot-to-Telegram)
3. HTTP progressive streaming (MP4 file stream) - bad format for streaming because of high start delay. This format doesn't work in all Safari browsers, but vrtc3 will automatically redirect it to HLS/fMP4 it this case.
...
*[New in v1.1.0](https://github.com/vtpl1/vrtc3/releases/tag/v1.1.0)*
...
The vrtc3 implementation differs from the standards and may not work with all players.
...
- some cameras has HTTP link with snapshots - vrtc3 can convert them to [MJPEG stream](#source-http)
...
- JPEG snapshots: `http://192.168.1.123:1984/api/frame.jpeg?src=camera1`
  ...
  - You can use `hardware`/`hw` param [read more](https://github.com/vtpl1/vrtc3/wiki/Hardware-acceleration)

**PS.** This module also supports streaming to the server console (terminal) in the **animated ASCII art** format ([read more](https://github.com/vtpl1/vrtc3/blob/master/internal/mjpeg/README.md)):
...
By default `vrtc3` starts the Web interface on port `1984` and RTSP on port `8554`, as well as use port `8555` for WebRTC connections. The three ports are accessible from your local network. So anyone on your local network can watch video from your cameras without authorization. The same rule applies to the Home Assistant Add-on.

This is not a problem if you trust your local network as much as I do. But you can change this behaviour with a `vrtc3.yaml` config:
...
- local access to API is not a problem for [Home Assistant Add-on](#vrtc3-home-assistant-add-on), because Hass runs locally on same server and Add-on Web UI protected with Hass authorization ([Ingress feature](https://www.home-assistant.io/blog/2019/04/15/hassio-ingress/))
...
vrtc3 can automatically detect which codecs your device supports for [WebRTC](#module-webrtc) and [MSE](#module-mp4) technologies.
...
There are no plans to embed complex transcoding algorithms inside vrtc3. [FFmpeg source](#source-ffmpeg) does a great job with this. Including [hardware acceleration](https://github.com/vtpl1/vrtc3/wiki/Hardware-acceleration) support.

But vrtc3 has some simple algorithms. They are turned on automatically, you do not need to set them up additionally.
...
By default WebRTC support only `PCMA/8000` and `PCMU/8000`. But vrtc3 can automatically resample PCMA and PCMU codec with with a different sample rate. Also vrtc3 can transcode `PCM` codec to `PCMA/8000`, so WebRTC can play it:
...
**vrtc3** automatically match codecs for you browser and all your stream sources. This called **multi-source 2-way codecs negotiation**. And this is one of the main features of this app.
...
- [OpenIPC](https://github.com/OpenIPC/firmware/tree/master/general/package/vrtc3) - Alternative IP Camera firmware from an open community
...
- [MMM-vrtc3](https://github.com/Anonym-tsk/MMM-vrtc3) - MagicMirror² Module
- [ring-mqtt](https://github.com/tsightler/ring-mqtt) - Ring devices to MQTT Bridge

**Distributions**

- [Alpine Linux](https://pkgs.alpinelinux.org/packages?name=vrtc3)
- [Arch User Repository](https://linux-packages.com/aur/package/vrtc3)
- [Gentoo](https://github.com/inode64/inode64-overlay/tree/main/media-video/vrtc3)
- [NixOS](https://search.nixos.org/packages?query=vrtc3)
- [Proxmox Helper Scripts](https://tteck.github.io/Proxmox/)
- [QNAP](https://www.myqnap.org/product/vrtc3/)
- [Synology NAS](https://synocommunity.com/package/vrtc3)
- [Unraid](https://unraid.net/community/apps?q=vrtc3)
...
[read more](https://github.com/vtpl1/vrtc3/wiki/Snapshot-to-Telegram)

## FAQ

**Q. What's the difference between vrtc3, WebRTC Camera and RTSPtoWebRTC?**

**vrtc3** is a new version of the server-side [WebRTC Camera](https://github.com/vtpl1/WebRTC) integration, completely rewritten from scratch, with a number of fixes and a huge number of new features. It is compatible with native Home Assistant [RTSPtoWebRTC](https://www.home-assistant.io/integrations/rtsp_to_webrtc/) integration. So you [can use](#module-hass) default lovelace Picture Entity or Picture Glance.

**Q. Should I use vrtc3 addon or WebRTC Camera integration?**

**vrtc3** is more than just viewing your stream online with WebRTC/MSE/HLS/etc. You can use it all the time for your various tasks. But every time the Hass is rebooted - all integrations are also rebooted. So your streams may be interrupted if you use them in additional tasks.

Basic users can use **WebRTC Camera** integration. Advanced users can use vrtc3 addon or Frigate 12+ addon.

**Q. Which RTSP link should I use inside Hass?**

You can use direct link to your cameras there (as you always do). **vrtc3** support zero-config feature. You may leave `streams` config section empty. And your streams will be created on the fly on first start from Hass. And your cameras will have multiple connections. Some from Hass directly and one from **vrtc3**.

Also you can specify your streams in **vrtc3** [config file](#configuration) and use RTSP links to this addon. With additional features: multi-source [codecs negotiation](#codecs-negotiation) or FFmpeg [transcoding](#source-ffmpeg) for unsupported codecs. Or use them as source for Frigate. And your cameras will have one connection from **vrtc3**. And **vrtc3** will have multiple connection - some from Hass via RTSP protocol, some from your browser via WebRTC/MSE/HLS protocols.

Use any config what you like.

**Q. What about lovelace card with support 2-way audio?**

At this moment I am focused on improving stability and adding new features to **vrtc3**. Maybe someone could write such a card themselves. It's not difficult, I have [some sketches](https://github.com/vtpl1/vrtc3/blob/master/www/webrtc.html).
